...
type UserORM struct {
	...
	Items       []*StoreItemORM `gorm:"foreignkey:UserId;association_foreignkey:Id"`
	Name        string
	Password    string
	Stats       *UserStatsORM `gorm:"foreignkey:UserStatsId;association_foreignkey:Id;preload:true"`
	UserStatsId *int64
}
...
func DefaultStrictUpdateUser(ctx context.Context, in *User, db *gorm1.DB) (*User, error) {
	...
	filterItems := StoreItemORM{}
	if ormObj.Id == "" {
		return nil, errors1.EmptyIdError
	}
	filterItems.UserId = new(string)
	*filterItems.UserId = ormObj.Id
	if err = db.Where(filterItems).Delete(StoreItemORM{}).Error; err != nil {
		return nil, err
	}
...
func DefaultListUserStats(ctx context.Context, db *gorm1.DB, f *query1.Filtering, s *query1.Sorting, p *query1.Pagination, fs *query1.FieldSelection) ([]*UserStats, error) {
	in := UserStats{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	...
	db, err = gorm2.ApplyCollectionOperators(ctx, db, &UserStatsORM{}, &UserStats{}, f, s, p, fs)
	if err != nil {
		return nil, err
	}
...
type UserStatsORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm1.DB, *[]UserStatsORM, *query1.Filtering, *query1.Sorting, *query1.Pagination, *query1.FieldSelection) error
}
...
// ListStats ...
func (m *UsersStatsDefaultServer) ListStats(ctx context.Context, in *ListUserStatsRequest) (*ListUserStatsResponse, error) {
	db := m.DB
	if custom, ok := interface{}(in).(UsersStatsUserStatsWithBeforeListStats); ok {
		var err error
		if db, err = custom.BeforeListStats(ctx, db); err != nil {
			return nil, err
		}
	}
	pagedRequest := false
	...
	res, err := DefaultListUserStats(ctx, db, in.Filter, in.OrderBy, in.Paging, in.Fields)
	if err != nil {
		return nil, err
	}
	var resPaging *query1.PageInfo
	...
	out := &ListUserStatsResponse{Results: res, Page: resPaging}
	if custom, ok := interface{}(in).(UsersStatsUserStatsWithAfterListStats); ok {
		var err error
		if err = custom.AfterListStats(ctx, out, db); err != nil {
			return nil, err
		}
	}
	return out, nil
...
// UsersStatsUserStatsWithBeforeListStats called before DefaultListStatsUserStats in the default ListStats handler
type UsersStatsUserStatsWithBeforeListStats interface {
	BeforeListStats(context.Context, *gorm1.DB) (*gorm1.DB, error)
}

// UsersStatsUserStatsWithAfterListStats called before DefaultListStatsUserStats in the default ListStats handler
type UsersStatsUserStatsWithAfterListStats interface {
	AfterListStats(context.Context, *ListUserStatsResponse, *gorm1.DB) error
}
